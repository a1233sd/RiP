...
def calc():
    return random.choice(["У", "C", "П"]) + str(random.randint(100, 999))
...
def add_reprimands():
    ...
    if not students.exists() or not moderators.exists():
        print("Выговоры не могут быть добавлены. Сначала добавьте студентов и модераторов.")
        return

    reasons = [
        "Пропуск лекций без уважительной причины",
        "Нарушение дисциплины на практическом занятии",
        "Уход с лекции без разрешения преподавателя",
        "Опоздание на занятие более чем на 15 минут",
        "Игнорирование указаний преподавателя на лекции"
    ]

    for student in random.sample(list(students), 5):  # Добавляем выговоры для 5 случайных студентов
        Reprimand.objects.create(
            student=student,
            reason=random.choice(reasons),
            issued_by=random.choice(moderators),
            status=random.choice([1, 2])
        )

    print("Выговоры добавлены")
